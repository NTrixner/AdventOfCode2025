...
@Slf4j
public class Playground implements ChallengeRunner<String> {
    public static final int BIGGEST_GROUPS_MULTIPLIED = 3;
    public static final int CLOSEST_NODES_COMBINED = 1000;
    ...
    public void run() {
        ...
        for(int i = 0; i < CLOSEST_NODES_COMBINED; i++) {
        ...
        groups.sort(Comparator.comparing(g -> g.getNodes().size(), reverseOrder()));
        double total = 1;
        for(int i = 0; i < BIGGEST_GROUPS_MULTIPLIED; i++) {
            total *= groups.get(i).getNodes().size();
        }

        log.info("Total distance multiplied: {}", total);
    }
}
